...
switch probe
    case '2DS'
        ...
        files = dir([filedir,'cat.DIMG.base*.2DS.V.proc.cdf']); 
        ...
        if (filenums>1)
            catDist([filedir,'SD.cat.DIMG.base*.2DS.V.proc.cdf']);
            system(['mv ',filedir,'SD.cat.DIMG.base.2DS.V.proc.cdf ',filedir,'SD.cat.DIMG.base',date,'.2DS.V.proc.cdf']);
        end

        %Generate H channel size dists for day
        files = dir([filedir,'cat.DIMG.base*.2DS.H.proc.cdf']);
        filenums = length(files);
        ...
        if (filenums>1)
            catDist([filedir,'SD.cat.DIMG.base*.2DS.H.proc.cdf']);
            system(['mv ',filedir,'SD.cat.DIMG.base.2DS.H.proc.cdf ',filedir,'SD.cat.DIMG.base',date,'.2DS.H.proc.cdf']);
        end
    case 'CIPG'
